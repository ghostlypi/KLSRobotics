...
* integral)/360);
         ...
         if(goal != 1)
         {
            ...
            if(eMeasure < (goal - (range/2)))
				...
				joystickArmValue = -0.4;
			System.out.println("PID: " + joystickArmValue);
			System.out.println("Goal: " + goal);
			System.out.println("");
		 }
		 else{
			System.out.println("Joystick: " + joystickArmValue);
			System.out.println("");
		 }
...
rightTalon.set(ControlMode.PercentOutput,joystickWheelSpeedValue);
		...
		boolean ultraCalebTriggerMode = joystick0.getRawButton(2);
		if(ultraCalebTriggerMode)
		 	myDrive.tankDrive(0.3 * joystickLValue, 0.3 * joystickRValue);
        else if(calebsTriggerMode)
